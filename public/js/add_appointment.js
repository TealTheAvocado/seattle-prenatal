...
// Citation for the add javascript using ajax: 
// Date: 11/09/2023
// Copied from CS340 OSU course:
// Source URL: https://github.com/osu-cs340-ecampus/nodejs-starter-app

// Get the objects we need to modify
let addAppointmentForm = document.getElementById('add-appointment-form-ajax');
...
toggleAddFormButton.addEventListener("click", function () {
    ...
    if (addContainer.style.display === 'block') {
        addAppointmentForm.scrollIntoView({ behavior: 'instant' });
    }
...
addAppointmentForm.addEventListener("submit", function (e) {
    ...
    let inputBillingCode = document.getElementById("input-billing-code");
    let inputDescription = document.getElementById("input-description");

    // Get the values from the form fields
    let nameValue = inputName.value;
    let billingCodeValue = inputBillingCode.value;
    let descriptionValue = inputDescription.value;

    // Put our data we want to send in a javascript object
    let data = {
        name: nameValue,
        billingCode: billingCodeValue,
        description: descriptionValue
    }
    
    // Setup our AJAX request
    var xhttp = new XMLHttpRequest();
    xhttp.open("POST", "/add-appointment-ajax", true);
...
addRowToTable = (data) => {
    ...
    let currentTable = document.getElementById("appointments-table");
    ...
    let billingCodeCell = document.createElement("TD");
