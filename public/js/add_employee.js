...
// Citation for the add employee javascript using ajax: 
// Date: 11/09/2023
...
let addEmployeeContainer = document.getElementById('add-container');
let toggleAddEmployeeFormButton = document.getElementById('toggle-add-form');

// Modify the objects
toggleAddEmployeeFormButton.addEventListener("click", function () {
    // Toggle the visibility of the form container if display is none, set it as block
    addEmployeeContainer.style.display = 'block';

    if (addEmployeeContainer.style.display === 'block') {
        addEmployeeForm.scrollIntoView({ behavior: 'instant' });
    }
});
